...
async function processVideoJob() {
  try {
    ...
    if (result.success && result.outputUrl) {
      ...
      // Extraire le chemin de destination depuis l'URL retournée
      const urlParts = result.outputUrl.split('/');
      const destinationPath = urlParts.slice(-2).join('/'); // Prend les 2 derniers segments (table/id_merged.mp4)
      
      console.log('📤 Fichier uploadé vers Supabase:', { 
